...
if (isset($_GET['campId'])) {
    ...
    while ($getNumFetch = mysqli_fetch_assoc($getNumExe)) {
        ...
        "tempName":"' . $tempName . '"
            ...
            $InDlrSQL = "Insert into whatsappDlr (bNum, MSISDN, ackId, delTime, delStatus)
            values ('917092122221', '91
            $custNum', '$transId', '$currentDateTime', 'request')";
            ...
            if ($headerType == 'None') {
                ...
                $urltemp = "http://$api_gateip:8080/api/$base_encrypt
                /insertoutboundtemplet?id=$transId&src=917092122221&dst=$temponenumber
                &timestamp=$timestamp_unix&type=Templet&msgType=Outbound&headerType=
                $headerType&bodyContent=$bodyContent&footerContent=$footerContent";
                if ($btnType == 'cta') {
                    if ($btnCta != 'N/A') {
                        $urltemp .= "&btnType=$btnType&btnCtaCpnTxt=
                        $btnCtaCpnTxt&btnCtaCpnCountry=$btnCtaCpnCountry&btnCtaCpnNum=
                        $btnCtaCpnNum";
                    }
                    if ($btnCtaVwTxt != 'N/A') {
            ...
            } elseif ($mediaType == 'DOC') {


                if ($fileExtension == 'PDF' || $fileExtension == 
                'DOCX' || $fileExtension == 'XLSX' || $fileExtension == 'XLS' || 
        ...
        "tempName":"' . $tempName . '"
            ...
            if ($headerType == 'Media') {
                ...
                &timestamp=$timestamp_unix&type=Templet&msgType=Outbound&headerType=
                $headerType&bodyContent=$bodyContent&footerContent=$footerContent
                &mediaType=$mediaType&s3_URL=$mediaUrl";
                if ($btnType == 'cta') {
                    $urltemp .= "&btnType=$btnType";
                    if ($btnCta != 'N/A') {
                        $urltemp .= "&btnCtaCpnTxt=$btnCtaCpnTxt
                        &btnCtaCpnCountry=$btnCtaCpnCountry&btnCtaCpnNum=$btnCtaCpnNum";
                    }
                    if ($btnCtaVwTxt != 'N/A') {
